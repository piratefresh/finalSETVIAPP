...
export default {
  ...
  Query: {
    getResourceStats: async (_, args) => {
      const data = await getEntireResourceStats(args.ResourceID);
      return data;
    }
  }
};

const getResourceStatsFromServer = async ResourceID => {
  const response = await fetch(
    `https://dev51.setvi.com/webapi/getresourceanalytics?resourceid=${ResourceID}&enddate=06/06/2019&startdate=01/01/2017`,
    {
      method: "post",
...
const getEntireResourceStats = async ResourceID => {
  const results = await getResourceStatsFromServer(ResourceID);
  if (results.length > 0) {
    return results.concat(await getEntireResourceStats(ResourceID + 1));
  } else {
    return results;
  }
};
