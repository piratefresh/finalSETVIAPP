...
export default {
  Query: {
    getUsersGroups: async (_, args) => {
      const response = await fetch(
        `https://dev51.setvi.com/webapi/getusersandgroups`,
        {
          method: "post",
...
// User: {
//     DateAdded: async (parent, args) => {
//       console.log(obj);
//       const date = await new Date(parseInt(obj.DateAdded.substr(6)));
//       const parsedDate = await dayjs(date).format("DD/MMM/YYYY");
//       return parsedDate;
//     },
//     LastActiveDate: (obj, args) => {
//       const date = new Date(parseInt(obj.LastActiveDate.substr(6)));
